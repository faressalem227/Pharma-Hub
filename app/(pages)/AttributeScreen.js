...
const AttributeItem = ({ item, index, size, handleDelete = () => {} }) => {
  console.log('item', item);

  return (
    <View
      className={`flex-row items-center justify-between py-4 ${index !== size - 1 ? 'border-b border-b-borderGray' : ''}`}>
      <Text className="font-tregular text-lg text-secndryText">{item.AttributeName}</Text>
...
const AttributeScreen = () => {
  ...
  const handleGetAttirbuteList = async () => {
    try {
      setIsLoading(true);
      const response = await api.get(`list?sp=attribute_list&GenderID=${user.gender}`);
  ...
  const handleGetUserAttribute = async () => {
    try {
      setIsLoading(true);
  ...
  const handleDeleteAtrribute = async (id) => {
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <AttributeItem
                  item={item}
                  index={index}
                  size={userAttributes.length}
                  handleDelete={handleDeleteAtrribute}
                />
              )}
            />
          </View>
