...
const Pharmacy = ({ item, index, size, handleDelete, handleView }) => {
  return (
    <View
      className={`py-4 ${index !== size - 1 ? 'border-b border-b-borderGray' : ''}`}
      key={item.ID}>
      <View className="flex-row items-center justify-between">
        <Text className="font-tregular text-lg text-secndryText">{item.PharmacyName}</Text>
      ...
      <Text className="mt-3 font-tregular text-sm text-secndryText">{item?.Address}</Text>
    </View>
  );
};

const SavedPahrmaciesScreen = () => {
  const { savedPharmacies, getsavedPharmacies, setPharmacy } = useContext(SearchContext);
  ...
  const handleDeletePh = async (id) => {
    try {
      setIsLoading(true);
      const response = await api.delete(`saved-pharmacies/${id}`);
  ...
  return (
    <View className="flex-1 bg-white p-4">
      <View className="flex-row">
        <Navback title="Saved pharmacies" />
      </View>

      {!isLoading && (
        <View>
          <FlatList
            data={savedPharmacies}
            keyExtractor={(item) => item.ID}
            renderItem={({ item, index }) => (
              <Pharmacy
                item={item}
                index={index}
                size={savedPharmacies.length}
                handleDelete={handleDeletePh}
              />
            )}
          />
        </View>
